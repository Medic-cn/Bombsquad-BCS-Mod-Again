...
def vote(pb_id, client_id, vote_type):
    ...
    if now < game_started_on + min_game_duration_to_start_vote:
        _ba.screenmessage("游戏才刚开始呢，别急着投票😣", transient=True,
                          clients=[client_id])
        return
    if len(voters) == 0:
        ba.internal.chatmessage(f"投票使用 {vote_type} 开始")
    ...
    if pb_id not in voters:
        voters.append(pb_id)
        _ba.screenmessage(f'感谢你的投票🫡让其他玩家发送 {vote_type} 就可以增加投票了😼', transient=True,
                          clients=[client_id])
        if vote_type == 'end':
            ...
            if activity is not None:
                with _ba.Context(activity):
                    _ba.screenmessage(f"还需要 {max_votes_required(len(active_players)) - len(voters)} 个投票才能使用 {vote_type}🎟️",
    ...
    if len(voters) >= max_votes_required(len(active_players)):
        ba.internal.chatmessage(f"{vote_type} 投票成功！🚩")
...
def update_vote_text(votes_needed):
    activity = _ba.get_foreground_host_activity()
    try:
        activity.end_vote_text.node.text = "====⏬投票跳过此关卡开始⏬====\n**🛎️需要 {} 个投票来跳过此关卡🛎️**\n**📨聊天框发送 'end' 来投票吧📨**".format(
            votes_needed)
    except:
        with _ba.Context(_ba.get_foreground_host_activity()):
