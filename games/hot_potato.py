...
class HotPotato(ba.TeamGameActivity[Player, ba.Team]):
    ...
    name = '热土豆'
    description = ('随机选取一位或两位玩家被标记\n'
                   '把标记传给其他玩家吧\n'
                   '在时间结束时被标记的玩家将死亡\n'
                   '争取胜利吧！炸队汉化组汉化')
    tips = [
        '除了用拳头，你还可以用炸弹把标记传给其他人.',
        '如果你没有被标记，不要轻易地掉出地图！\n否则你会有一段冻结时间',
        '掉出地图可以帮助你逃跑，但不要一直掉！\n冻结时间每次都会增长',
        '冻结时间只对健康玩家有用，如果你被标记，你就没有冻结时间',
        '每次掉出地图都会增长你的冻结时间',
        '既然掉出地图会被冻结，为什么不把其他健康玩家丢出去呢（doge）',
        '被标记的玩家掉出地图后没有冻结时间',
        '或许你可以把其他人丢出地图来保护自己？',
        '当然是人越多越好玩啦⎛⎝≥⏝⏝≤⎛⎝',
        '红色的炸弹是来自被标记玩家的！\n小心那些炸弹！',
    ...
    available_settings = [
        ...
        ba.BoolSetting('被标记玩家使用触感炸弹', default=False),
    ...
    def mark(self, target: Player) -> None:
        ...
        ba.emitfx(position=target.actor.node.position,
                  ...
                  scale=1.0,
                  spread=1.0)
        if bool(self.settings['被标记玩家使用触感炸弹']):
    ...
    def _eliminate_tick(self) -> None:
        ...
        if self.elimination_timer_display > 1:
            ...
            # 生存时间 is up! Let's eliminate all marked players.
            self.elimination_timer_display -= 1  # Decrease our timer by one second.
    ...
    def new_mark(self) -> None:
        ...
        all_victims = []
        # Let's mark TWO players at once if there's 3 or more players. Helps with the pacing.
        multi_choice = len(possible_targets) > 3
        ...
        self.elimination_timer_display = self.settings['生存时间']
