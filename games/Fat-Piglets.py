"""Credits to the original creator of the Fat-Pigs Game. Updated for More Minigames by R1chK1ds"""
...
def ba_get_levels():
	return [ba._level.Level(
            '胖猪',
			gametype=BombShowerGame,
			settings={},
			preview_texture_name='courtyardPreview'), ba._level.Level(
            '史诗级胖猪',
			gametype=BombShowerGame,
...
# ba_meta export game
class BombShowerGame(ba.TeamGameActivity[Player, Team]):
    ...
    def on_begin(self) -> None:
     ...
     #   gnode.vignette_inner = (0.9, 0.9, 0.9)
        ...
        ba.timer(4, self._newbot, repeat=True)

    def _newbot(self) -> None:
        pos = (-5 + 5 * random.random(), 4,
               -5 + 5 * random.random())
    ...
    def spawn_player(self, player: Player) -> ba.Actor:
        ...
        #cerdo gordo
        spaz.node.color_mask_texture = ba.gettexture('melColorMask')
        spaz.node.color_texture = ba.gettexture('melColor')
        spaz.node.head_model = ba.getmodel('melHead')
        spaz.node.hand_model = ba.getmodel('melHand')
        spaz.node.torso_model = ba.getmodel('melTorso')
        spaz.node.pelvis_model = ba.getmodel('kronkPelvis')
        spaz.node.upper_arm_model = ba.getmodel('melUpperArm')
        spaz.node.forearm_model = ba.getmodel('melForeArm')
        spaz.node.upper_leg_model = ba.getmodel('melUpperLeg')
        spaz.node.lower_leg_model = ba.getmodel('melLowerLeg')
        spaz.node.toes_model = ba.getmodel('melToes')
        spaz.node.style = 'mel'
        # Sounds cerdo gordo
        mel_sounds = [ba.getsound('mel01'), ba.getsound('mel02'),ba.getsound('mel03'),ba.getsound('mel04'),ba.getsound('mel05'),
                      ba.getsound('mel06'),ba.getsound('mel07'),ba.getsound('mel08'),ba.getsound('mel09'),ba.getsound('mel10')]
        ...
        spaz.node.death_sounds = [ba.getsound('melDeath01')]
        spaz.node.fall_sounds = [ba.getsound('melFall01')]
...
campaign.addlevel(
_level.Level(name='Impact Shower',
                 displayname='impact',
                 gametype=BombShowerGame,
                 settings={},
