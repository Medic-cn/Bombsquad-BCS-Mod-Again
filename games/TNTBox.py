...
class TNTBox(ba.Actor):
    ...
    def __init__(self,
        ...
        if modeltype == 1:
            ...
            self.node = ba.newnode(
                ...
                attrs={
                    ...
                    'body': 'crate',
                    'body_scale': 1,
                    'model_scale': self._mymodel_scale,
                    'shadow_size': 0.3,
                    'color_texture': ba.gettexture('tnt'),
                    'is_area_of_interest': True,
                    'reflection': 'soft',
        ...
        ba.animate(self.node, 'model_scale', {
            0: 0,
            0.2: self._mymodel_scale * 1.1,
            0.26: self._mymodel_scale})
    ...
    def animate_model(self) -> None:
        if not self.node:
            return None
        ba.animate(self.node, 'model_scale', {
            0: self._mymodel_scale,
            0.08: self._mymodel_scale * 0.9,
            0.15: self._mymodel_scale})
...
class TNTTeamGame(ba.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        self._tnt_hitpoints = int(settings['TNT击中点'])
        self._model_type = float(settings['物体形状'])
    ...
    def get_instance_description(self) -> Union[str, Sequence]:
        return '摧毁敌方建筑，记得要保护自己的\n炸队汉化组汉化 网站www.inker.ga'
