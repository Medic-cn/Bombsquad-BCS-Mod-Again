...
async def monitor_network():
    ...
    while True:
        ...
        zaqr.sendLog("CPU: {psutil.cpu_percent()}% 内存: {psutil.virtual_memory().percent}% 上传: {upload_speed} 下载: {download_speed}")
        logger.log(f"CPU: {psutil.cpu_percent()}% 内存: {psutil.virtual_memory().percent}% 上传: {upload_speed} 下载: {download_speed}")
