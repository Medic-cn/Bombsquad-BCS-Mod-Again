#this animate made by Medic and wsdx233
#作者：inkMedic  由衷感谢wsdx233
#严禁抄袭 代码未开源 违者必究 请遵守开源规则
#Animater for blackTips
import _ba
import ba
reTime = False
def animater(mgs,name,coin,cmd):
        ...
        with ba.Context(activity):
            ba.animate_array(activity.image.node, 'position', 2, {
                0.0*s: (posi[0], posi[1]),
                0.05*s: (posi[0]+30,posi[1]),
                0.1*s: (posi[0]+45,posi[1]),
                0.15*s: (posi[0]+55,posi[1]),
                0.2*s: (posi[0]+60,posi[1]),
                0.25*s: (posi[0]+63,posi[1]),
                0.3*s: (posi[0]+65,posi[1]),
                0.35*s: (posi[0]+67,posi[1]),
                0.4*s: (posi[0]+68,posi[1]),
                4.9*s: (posi[0]+68,posi[1]),
                4.95*s: (posi[0]+60,posi[1]),
                5.0*s: (posi[0]+40,posi[1]),
                5.1*s: (posi[0]+10,posi[1]),
                5.15*s:(posi[0],posi[1])
            })
        with ba.Context(activity):
            ba.animate(activity.image.node, 'opacity', {
                0.0*s: (0),
                0.1*s: (0.2),
                0.2*s: (0.6),
                0.3*s: (0.8),
...
def speed(s_pos,e_pos,t):
    s_plus = 2/5*(s_pos-e_pos)
    s_costom = 3/5*(s_pos-e_pos)
    re = []
    n_pos = s_pos
    for i in range(20,1):
        t_play = t/i
        if n_pos >= s_pos+s_costom:
            n_pos 
